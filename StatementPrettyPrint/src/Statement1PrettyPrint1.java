...
public final class Statement1PrettyPrint1 extends Statement1 {
    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            case BLOCK: {
                /*
                 * for block, recursively call prettPrint on each block inside
                 * the block.
                 */
            ...
            case IF: {
                ...
                Condition condition = this.disassembleIf(statement);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(condition) + " THEN");
                statement.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);

                out.println("END IF");
                this.assembleIf(condition, statement);
                break;
            }
            case IF_ELSE: {

                Statement statement1 = this.newInstance();
                Statement statement2 = this.newInstance();
                Condition condition = this.disassembleIfElse(statement1,
                        statement2);
                printSpaces(out, offset);

                out.println("IF " + toStringCondition(condition) + " THEN");
                statement1.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);

                out.println("ELSE");
                statement2.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);

                out.println("END IF");
                this.assembleIfElse(condition, statement1, statement2);
                break;
            }
            case WHILE: {
                Statement statement = this.newInstance();
                Condition condition = this.disassembleWhile(statement);
                printSpaces(out, offset);
                ...
                this.assembleWhile(condition, statement);
                break;
            }
            case CALL: {
