...
public final class RSSProcessing {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        XMLTree firstLine = item.child(0);
        XMLTree secondLine = item.child(0);
        for (int i = 0; i < item.numberOfChildren(); i++) {
            if (item.child(i).label().equals("title")) {
                firstLine = item.child(i).child(0);
                break;
            } else if (item.child(i).label().equals("description")) {
                firstLine = item.child(i).child(0);
            }
        }
        for (int j = 0; j < item.numberOfChildren(); j++) {
            if (item.child(j).label().equals("link")) {
                secondLine = item.child(j).child(0);
            }

        }
        out.println("Title: " + firstLine);
        out.println("Link: " + secondLine);
    ...
    public static void main(String[] args) {
        ...
        out.println(getChildElement(channel, "link"));
        /*
         * TODO: #2 - output title, link, and description
         */
        out.println("Title: " + channel.child(0).child(0));
        out.println("Description: " + channel.child(2).child(0));
        out.println("Link: " + channel.child(1).child(0));
        /*
