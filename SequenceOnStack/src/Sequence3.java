...
public class Sequence3<T> extends SequenceSecondary<T> {
    ...
    private static <T> void setLengthOfLeftStack(Stack<T> leftStack,
        ...
        if (leftStack.length() > newLeftLength) {
            Stack<T> leftStackReverse = leftStack.newInstance();
            while (leftStack.length() > 0) {
                leftStackReverse.push(leftStack.pop());
            }
            while (leftStackReverse.length() > newLeftLength) {
                rightStack.push(leftStackReverse.pop());
            }
            while (leftStackReverse.length() > 0) {
                leftStack.push(leftStackReverse.pop());
            }
            ...
            while (leftStackReverse.length() < newLeftLength) {
                leftStackReverse.push(rightStack.pop());
            }
            while (leftStackReverse.length() > 0) {
                leftStack.push(leftStackReverse.pop());
            }
