...
public class SortingMachine4<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void partition(Queue<T> q, T partitioner,
        ...
        while (q.length() > 0) {
            T current = q.dequeue();
            if (order.compare(current, partitioner) <= 0) {
                front.enqueue(current);
            } else {
                back.enqueue(current);
            }
        }
    }
    ...
    private static <T> boolean isComplete(BinaryTree<T> t) {
        ...
        if (t.size() > 0) {
            ...
            if (lt.size() < rt.size()) {
                ...
                isComplete = isComplete(lt) && isComplete(rt);
            }
    ...
    private static <T> void sort(Queue<T> q, Comparator<T> order) {
        ...
        if (q.length() > 1) {
            ...
            partition(q, entry, front, back, order);
            /*
    ...
    public final void changeToExtractionMode() {
        ...
        //sort this.entries in extraction mode.
