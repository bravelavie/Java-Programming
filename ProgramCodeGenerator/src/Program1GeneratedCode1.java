    ...
    private static void generateCodeForStatement(Statement s,
        ...
        switch (s.kind()) {
            case BLOCK: {
                ...
                // case block will always need a loop.
            ...
            case IF_ELSE: {
                ...
                int jump = cp.length();
                cp.add(cp.length(), dummy);
                generateCodeForStatement(x, context, cp);
                cp.replaceEntry(jump, cp.length());
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                cp.add(cp.length(), dummy);
                generateCodeForStatement(y, context, cp);
                cp.replaceEntry(jump, cp.length());
            ...
            case WHILE: {
                ...
                cp.replaceEntry(jump, cp.length());
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                cp.add(cp.length(), conditionLine);
                s.assembleWhile(c, b);
                break;
            }
            case CALL: {
                ...
                switch (str) {
                    case "move": {
                        byteCode = Instruction.MOVE.byteCode();
                    ...
                    default: {
                        if (context.hasKey(str)) {
                            Statement a = context.value(str);
                            generateCodeForStatement(a, context, cp);
                        } else {
