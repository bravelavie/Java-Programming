...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        generateElements(SEPARATORS, seperators);
        if (seperators.contains(sub.charAt(0))) {
            for (int j = 0; j < sub.length(); j++) {
                if (seperators.contains(sub.charAt(j))) {
                    result += sub.charAt(j);
                } else {
                    break;
                }
            ...
            for (int i = 0; i < sub.length(); i++) {
                if (!seperators.contains(sub.charAt(i))) {
                    result += sub.charAt(i);
                } else {
                    break;
                }
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (position < inputLine.length()) {
                String token = nextWordOrSeparator(inputLine, position);
                //if not separator, add to tokens queue.
