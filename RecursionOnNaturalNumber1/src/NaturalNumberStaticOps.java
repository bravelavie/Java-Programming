...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        NaturalNumber m = n.newInstance();
        m.copyFrom(n);

        NaturalNumber k = new NaturalNumber2(999);
        if (m.compareTo(k) > 0) {
            int single = m.divideBy10();
            int ten = m.divideBy10();
            int hundred = m.divideBy10();
            printWithCommas(m, out);
            out.print("," + hundred + ten + single);
        } else {
            out.print(m);
        }
    ...
    public static String toStringWithCommas(NaturalNumber n) {
        ...
        if (m.compareTo(k) > 0) {
            ...
            int ten = m.divideBy10();
            int hundred = m.divideBy10();
            s = toStringWithCommas(m) + "," + hundred + ten + single;
        } else if (m.isZero()) {
            s = "0";
        } else {
            while (!m.isZero()) {
