...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String str2Cut = str2.substring(overlap, str2.length());
        String strCombined = str1 + str2Cut;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        while (temp.size() > 0) {
            ...
            for (String y : strSet) {
                if (y.contains(x) || x.contains(y)) {
                    Reporter.fatalErrorToConsole("Error: Set contains substring pairs");
                }
            }
            strSet.add(x);
        }
        /*
         * Check if input string is substring of any existing string in the set
         */
        ...
        if (!isSubStr) {
            /*
             * Check if existing string in the set is substring of input string,
             * remove if true
             */
            Set<String> temp1 = strSet.newInstance();
            temp1.transferFrom(strSet);
            while (temp1.size() > 0) {
                String s = temp1.removeAny();
                if (!str.contains(s)) {
                    strSet.add(s);
                }
            }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(output, input.nextLine());
        }
        return output;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            /*
             * replace current char with line separator when it is ~
             */
            if (current == '~') {
                out.println();
            } else {
