...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(Array<Integer> array, int top, int last) {
        ...
        int leftChild = array.entry(leftIndex);
        int rightChild = array.entry(rightIndex);

        if (leftIndex <= last && (leftChild < array.entry(exchangeIndex))) {
            exchangeIndex = leftIndex;
        }

        if (rightIndex <= last && (rightChild < array.entry(exchangeIndex))) {
            exchangeIndex = rightIndex;
        }

        if (exchangeIndex != top) {
            array.exchangeEntries(top, exchangeIndex);
            siftDown(array, exchangeIndex, last);
        }
    }
