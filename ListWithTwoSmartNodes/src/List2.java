...
public class List2<T> extends ListSecondary<T> {
    ...
    private void createNewRep() {
        //one new preFront node with all references
    ...
    public final void addRightFront(T x) {
        ...
        /*
         * check if right list is empty before add, then this.finish need to
         * point to added node
         */
    ...
    public void retreat() {
        ...
        /*
         * move p to the second last left node, poor performance since it take
         * O(n) time in worst case
         */
        for (int i = 0; i < this.leftLength - 1; i++) {
            p = p.next;
        }
        //set p as last left node
        this.lastLeft = p;
        this.leftLength--;
        this.rightLength++;
    }
