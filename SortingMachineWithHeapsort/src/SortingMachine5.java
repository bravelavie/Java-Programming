...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        int exchangeIndex = top;

        if (leftIndex <= last
                && order.compare(array.entry(leftIndex),
                        array.entry(exchangeIndex)) < 0) {
            exchangeIndex = leftIndex;
        }

        if (rightIndex <= last
                && order.compare(array.entry(rightIndex),
                        array.entry(exchangeIndex)) < 0) {
            exchangeIndex = rightIndex;
        }

        if (exchangeIndex != top) {
            array.exchangeEntries(top, exchangeIndex); //sift top down by one level
            siftDown(array, exchangeIndex, last, order); //repeat the process to lower level
        }
    }
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * for heap structure, changeToExtractionMode only build a heap, but not
    ...
    public final T removeFirst() {
        ...
        /* removeFirst will maintain the heap structure after the removed */
